...
$STEDGEAI_CORE_DIR/Utilities/linux/stedgeai analyze --target stm32n6 --name network -m ${INPUT_MODEL} --st-neural-art "n6-flash-${BASE_NAME_2}@config/user_neuralart.json" --verbosity 3
$STEDGEAI_CORE_DIR/Utilities/linux/python $STEDGEAI_CORE_DIR/scripts/N6_scripts/n6_loader.py --build-config N6-DK
$STEDGEAI_CORE_DIR/Utilities/linux/stedgeai validate --target stm32n6 --name network -m ${INPUT_MODEL} --st-neural-art "n6-flash-${BASE_NAME_2}@config/user_neuralart.json" --verbosity 3 --mode target -d serial:921600 -vi "${VALIDATION_DATA}"
